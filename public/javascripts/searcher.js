...
var Searcher = Class.$extend({
    ...
    handleSearch : function(json, onEach, onNone, mode) {
        ...
        // call onNone if results array is empty
        if (json.results.length == 0) {
            console.log("NO SEARCH RESULTS", mode);
        ...
        var searchFurther = function(uri, mode) {
            return $.getJSON(
                ...
                function(json) {
                    ...
                    that.handleSearch(json, onEach, undefined, mode);
                }
            );
        };
        ...
        if (mode === "refresh") {
            console.log(d, "schedule refresh_url", json.refresh_url);
            scheduleRefresh();
        }
        else if (mode === "next_page") {
            console.log(d, "schedule next_page", json.next_page);
            scheduleNextPage();
        }
        else {
            console.log(d, "schedule both", json.refresh_url, json.next_page);
            scheduleRefresh();
            scheduleNextPage();
        }
